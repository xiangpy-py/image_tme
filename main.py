...
from src.data import DatasetAnalyzer, DatasetSplitter
...
def cmd_infer(args: argparse.Namespace) -> None:
    ...
    for marker in MARKERS:
        value = getattr(args, f"ckpt_{marker.replace('-', '_')}")
        if value is not None:
            overrides[marker] = value

    Predictor.run_experiment(ConfigManager.load(args.config), args.exp, overrides)
