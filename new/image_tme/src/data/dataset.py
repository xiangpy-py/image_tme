...
from .constants import CONTEXT_DIR_MARKER, IMAGE_EXTENSIONS, MARKERS, SOURCE_MARKER
    ...
    def discover_marker_dirs(
        ...
        for child in sorted(root_path.iterdir()):
            if not child.is_dir() or child.name == split:
                continue
            # 跳过上下文图根目录：其内容与源数据同构，误当器官会重复计数。
            if (child / CONTEXT_DIR_MARKER).is_file():
                continue
...
class DatasetAnalyzer:
    ...
    def analyze(cls, root: str, output: str = "data/statistics.json") -> Dict[str, Any]:
        ...
        split_dirs = [
            ...
            if p.is_dir()
            and p.name in ("train", "test")
            # 排除上下文图目录（其父目录带生成器写入的标记文件）。
            and not (p.parent / CONTEXT_DIR_MARKER).is_file()
        ]
