...
class Ensembler:
    ...
    def __init__(
        ...
        # 与 Predictor 共用同一配置开关：TTA 在模型集成之前先做，
        # 两重平均（视图平均 + 模型平均）的收益基本正交。
            ...
            for batch in tqdm(test_loader, desc=f"ensemble [{marker}]"):
                ...
                for model in models:
                    if self.conditional:
                        ...
                        predictions.append(
                            Predictor.tta_forward(model, inputs, idx, enabled=self.tta)
                        )
                    else:
                        predictions.append(
                            Predictor.tta_forward(model, inputs, enabled=self.tta)
                        )
